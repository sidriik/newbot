...
EMOJI = {
    "search": "🔍", "plus": "➕", "list": "📋", "help": "❓", "home": "🏠", 
    "book": "📚", "info": "ℹ️", "read": "📖", "star": "⭐", "prev": "⬅️", 
    "hello": "👋", "wow": "🎉", "user": "👤", "chart": "📊", "cross": "❌",
    "folder": "📂", "check": "✅", "calendar": "📅", "trophy": "🏆", "mail": "📬"
}
...
async def button_handler(update: Update, context):
    ...
    if data == "main_menu":
        ...
        if not books:
            ...
            await query.edit_message_text(f"{EMOJI['mail']} У тебя пока нет книг.", reply_markup=InlineKeyboardMarkup(keyboard))
            return
        ...
        for book in popular:
            keyboard_buttons.append([
                InlineKeyboardButton(f"{EMOJI['book']} {book.get_short()}", callback_data=f"add_{book.id}")
            ])
        ...
        await query.edit_message_text(f"{EMOJI['book']} Выбери книгу для добавления:", reply_markup=InlineKeyboardMarkup(keyboard_buttons))
        ...
        if not planned:
            ...
            await query.edit_message_text(f"{EMOJI['mail']} Нет запланированных книг.", reply_markup=InlineKeyboardMarkup(keyboard))
            return
        
        keyboard_buttons = []
        for book in planned[:5]:
            keyboard_buttons.append([
                InlineKeyboardButton(f"{EMOJI['book']} {book.title[:15]}...", callback_data=f"start_{book.book_id}")
            ])
        
        keyboard_buttons.append([InlineKeyboardButton(f"{EMOJI['book']} Все книги", callback_data="mybooks")])
        keyboard_buttons.append([InlineKeyboardButton(f"{EMOJI['prev']} Назад", callback_data="main_menu")])
        
        await query.edit_message_text(f"{EMOJI['book']} Выбери книгу для чтения:", reply_markup=InlineKeyboardMarkup(keyboard_buttons))
...
{EMOJI['cross']} Брошено: {stats['dropped']}"""
        ...
        if not completed:
            ...
            await query.edit_message_text(f"{EMOJI['mail']} Нет прочитанных книг для оценки.", reply_markup=InlineKeyboardMarkup(keyboard))
            return
        ...
        for book in completed[:3]:
            if book.rating:
                keyboard_buttons.append([
                    InlineKeyboardButton(f"{EMOJI['star']} {book.rating}/5 - {book.title[:10]}...", callback_data="no_action")
                ])
            else:
                keyboard_buttons.append([
                    InlineKeyboardButton(f"{EMOJI['book']} {book.title[:15]}...", callback_data=f"rateshow_{book.book_id}")
                ])
        
        keyboard_buttons.append([InlineKeyboardButton(f"{EMOJI['book']} Мои книги", callback_data="mybooks")])
        keyboard_buttons.append([InlineKeyboardButton(f"{EMOJI['prev']} Назад", callback_data="main_menu")])
        
        await query.edit_message_text(f"{EMOJI['star']} Выбери книгу для оценки:", reply_markup=InlineKeyboardMarkup(keyboard_buttons))
...
/addbook - Добавить новую книгу в каталог"""
        ...
        if genre == "input":
            await query.edit_message_text(f"{EMOJI['search']} Введи название или автора:", reply_markup=InlineKeyboardMarkup([
...
Статус: Запланировано""",
        ...
        try:
            ...
            if not user_manager.has_book(user_db_id, book_id):
                await query.edit_message_text(f"{EMOJI['cross']} У тебя нет этой книги.", reply_markup=InlineKeyboardMarkup([
...
async def show_search_menu(upd):
    ...
    for i in range(0, min(len(genres), 6), 2):
        ...
        if i + 1 < len(genres):
            row.append(InlineKeyboardButton(f"{EMOJI['folder']} {genres[i+1]}", callback_data=f"search_{genres[i+1]}"))
        keyboard_buttons.append(row)
    
    keyboard_buttons.append([InlineKeyboardButton(f"{EMOJI['search']} Поиск по названию", callback_data="search_input")])
    keyboard_buttons.append([InlineKeyboardButton(f"{EMOJI['home']} Главное меню", callback_data="main_menu")])
